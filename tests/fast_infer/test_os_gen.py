...
def test_tau(num_gpus):
    ...
    engine.save_video(video, f"./test_outputs/stu/{output_title}.mp4")


@empty_cache
def test_base(num_gpus):
    config = OpenSoraConfig(num_gpus=num_gpus, num_sampling_steps=30)
    engine = VideoSysEngine(config)
    ...
    output_title = prompt.replace(" ", "_")[:20]
    engine.save_video(video, f"./test_outputs/base/{output_title}.mp4")
