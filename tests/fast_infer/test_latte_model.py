...
def test_latte_block(device):
    ...
    timestep = torch.randn(B * T, C, dtype=hs.dtype, device=device)
...
def test_latte_model(device):
    ...
    base_model = LatteT2V(**init_kwargs).to(device)
    test_model = TestModel(**init_kwargs).to(device)

    for bp, tp in zip(base_model.parameters(), test_model.parameters()):
        tp.data.copy_(bp.data)

    B, T, C, H, W = 2, 16, 4, 128, 128
    S, D = 1024, 1152
    hs = torch.randn(B, T, C, H, W, dtype=base_model.pos_embed.proj.weight.dtype, device=device)
    encoder_hs = hs.view(B * T, 6, D, dtype=hs.dtype, device=device)
    timestep = torch.tensor([2, 2], dtype=hs.dtype, device=device).view(B, 1)
    ...
    for i in range(depth):
        ...
        cache_list.append((att1_cache, att2_cache), att3_cache)
