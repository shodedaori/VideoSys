...
from videosys.schedulers.scheduling_tau_cogvideox import CogVideoXDDIMScheduler
...
class CogVideoXTauConfig:
    ...
    def __init__(
        ...
        self.enable_pab = False

        self.ti_coef = ti_coef
        self.ti_filter = ti_filter


class CogVideoXTauPipeline(VideoSysPipeline):
    ...
    def __init__(
        ...
        if transformer is None:
            transformer = CogTauTransformer3DModel.from_pretrained(config.model_path, subfolder="transformer", torch_dtype=self._dtype)
        if vae is None:
        ...
        self.coef = config.ti_coef
        self.filter = config.ti_filter
